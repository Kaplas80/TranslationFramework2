...
using TF.IO;
using YakuzaGame.Core;

namespace YakuzaGame.Files
{
    public partial class UIParFile
    {
        private UIParFile()
        {
        }
        ...
        {
            ExtractPar(inputPath, outputFolder);

            var surfboard = Path.Combine(outputFolder, "surfboard");
            var newParFiles = Directory.GetFiles(surfboard, "*.par", SearchOption.TopDirectoryOnly);
            foreach (var file in newParFiles)
            {
                var newOutputPath = Path.Combine(surfboard, $"{Path.GetFileName(file)}.unpack");
                ExtractPar(file, newOutputPath);
            }
        }

        private static void ExtractPar(string inputPath, string outputFolder)
        {
            Directory.CreateDirectory(outputFolder);

            var parFile = new UIParFile();
        ...
        {
            var surfboard = Path.Combine(inputFolder, "surfboard");
            var unpackedParFolders = Directory.GetDirectories(surfboard, "*.par.unpack", SearchOption.TopDirectoryOnly);
            foreach (var folder in unpackedParFolders)
            {
                var tempFile = folder.Substring(0, folder.Length - 7);
                RepackPar(folder, tempFile, useCompression);
            }

            RepackPar(inputFolder, outputPath, useCompression);
        }

        private static void RepackPar(string inputFolder, string outputPath, bool useCompression)
        {
